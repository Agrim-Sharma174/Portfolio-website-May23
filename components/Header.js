    ...
    <motion.header {...FadeAnimation} className="fixed left-0 -top-12 z-40 flex w-full justify-between pt-24 md:top-0">
      ...
      <div className="relative flex w-1/2 flex-col items-end justify-end md:flex-row">
        ...
        <div className={`absolute top-12 right-0 md:relative md:top-0 md:mr-10 md:w-11/12 lg:mr-40 lg:w-1/2 ${!open ? 'hide' : 'show'}`}>
          <nav className="navbar md:flex md:flex-col md:absolute md:-left-[800px] md:top-14 md:space-y-5">
            {navLinks.map((link) => (
              <Link key={link.title} href={link.path}>
                <a className={`nav-items md:backdrop-blur-xl md:hover:backdrop-blur-sm md:hover:bg-white md:hover:bg-opacity-5 md:w-20 md:h-20 md:rounded-full md:flex justify-center md:items-center md:transition md:duration-100 md:shadow-gray-50 md:hover:shadow-gray-50 md:shadow-sm md:hover:shadow-lg  ${router.pathname === link.path ? 'text-primary-light' : ''}`}>{link.title}</a>
              </Link>
            ))}
          </nav>
        </div>
